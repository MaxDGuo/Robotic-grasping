...
class GraspRectangle:
    ...
    def rotate(self, angle, center):
        ...
        """
        # Ensure angle is a scalar (handles numpy arrays of shape (1,))
        angle = float(np.asarray(angle))
        R = np.array([
            [np.cos(-angle), np.sin(-angle)],
            [-1 * np.sin(-angle), np.cos(-angle)],
        ])
        c = np.asarray(center).reshape((1, 2))
        pts = np.asarray(self.points, dtype=float)
        self.points = ((np.dot(R, (pts - c).T)).T + c).astype(int)
    ...
    def zoom(self, factor, center):
        ...
        """
        # Ensure factor is a scalar (handles numpy length-1 arrays)
        factor = float(np.asarray(factor))
        T = np.array([
            [1 / factor, 0],
            [0, 1 / factor]
        ])
        c = np.asarray(center).reshape((1, 2))
        pts = np.asarray(self.points, dtype=float)
        self.points = ((np.dot(T, (pts - c).T)).T + c).astype(int)
