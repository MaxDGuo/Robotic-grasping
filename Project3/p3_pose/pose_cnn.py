...
class SegmentationBranch(nn.Module):
    ...
    def __init__(self, num_classes = 10, hidden_layer_dim = 64):
        ...
        self.conv1 = nn.Conv2d(in_channels=512, out_channels=hidden_layer_dim, kernel_size=1)
        nn.init.kaiming_normal_(self.conv1.weight)
        nn.init.zeros_(self.conv1.bias)
        self.relu1 = nn.ReLU()

        self.conv2 = nn.Conv2d(in_channels=512, out_channels=hidden_layer_dim, kernel_size=1)
        nn.init.kaiming_normal_(self.conv2.weight)
        nn.init.zeros_(self.conv2.bias)
        self.relu2 = nn.ReLU()
        
        # TODO 2: 在这里你需要完成最后一个卷积层的初始化，self.conv3 是最终的卷积层，负责将经过前两层的特征图映射到最终类别数 + 背景类别的数量
        self.conv3 = nn.Conv2d(in_channels=hidden_layer_dim, out_channels=self.num_classes + 1, kernel_size=1)
    ...
    def forward(self, feature1, feature2):
        ...
        # TODO 4: 执行卷积操作并进行 ReLU 激活
        x1 = self.relu1(self.conv1(feature1))  # feature1 → 1x1卷积 → ReLU
        x2 = self.relu2(self.conv2(feature2))  # feature2 → 1x1卷积 → ReLU
        # 上采样x2（2倍），使其尺寸与x1一致，然后元素相加融合特征
        temp = nn.functional.interpolate(x2, scale_factor=2, mode='bilinear', align_corners=False) + x1

        # TODO 5: 上采样到原始图像尺寸 (480, 640)
        up_sample = nn.functional.interpolate(temp, size=(480, 640), mode='bilinear', align_corners=False)

        x3 = self.relu3(self.conv3(up_sample)) # 最终卷积,得到 (B, 11, 480, 640)

        probability = self.softmax(x3)  # 通道维度softmax，得到每个像素的类别概率
        segmentation = torch.argmax(probability, dim=1)  # 取概率最大的类别，得到 (B, 480, 640)
        bbx = self.label2bbx(segmentation)  # 从分割结果中提取边界框po
...
class TranslationBranch(nn.Module):
    ...
    def __init__(self, num_classes = 10, hidden_layer_dim = 128):
        ...
        self.conv3 = nn.Conv2d(in_channels=hidden_layer_dim, out_channels=3 * num_classes, kernel_size=1)
        nn.init.kaiming_normal_(self.conv3.weight)
        nn.init.zeros_(self.conv3.bias)
    ...
    def forward(self, feature1, feature2):
        ...
        # TODO 7: 完成卷积层和 ReLU 操作
        x1 = self.relu1(self.conv1(feature1))  # feature1 → 1x1卷积 → ReLU
        x2 = self.relu2(self.conv2(feature2))  # feature2 → 1x1卷积 → ReLU

        # TODO 8: 对 feature2 上采样并与 feature1 逐元素相加
        # 上采样x2到x1的空间尺寸（2倍缩放），双线性插值
        x2_upsampled = nn.functional.interpolate(x2, scale_factor=2, mode='bilinear', align_corners=False)
        temp = x1 + x2_upsampled

        x3 = self.conv3(temp)  
        # 上采样到原始图像尺寸(480,640)，输出平移预测图
        translation = nn.functional.interpolate(x3, size=(480, 640), mode='bilinear', align_corners=False)
...
class RotationBranch(nn.Module):
    ...
    def __init__(self, feature_dim = 512, roi_shape = 7, hidden_dim = 4096, num_classes = 10):
        ...
        # RoIPool参数：(输出尺寸, 空间缩放因子)
        # feature1是原图1/8缩放，feature2是1/16缩放，对应spatial_scale
        self.roi1 = RoIPool((roi_shape, roi_shape), spatial_scale=1/8)
        self.roi2 = RoIPool((roi_shape, roi_shape), spatial_scale=1/16)

        # TODO 10: 完成全连接层 lin1 的初始化
        # 输入维度：feature_dim * roi_shape * roi_shape（RoI池化后的特征展平）
        ...
        # 输出维度：4*num_classes（每个类别预测四元数的4个分量）
    ...
    def forward(self, feature1, feature2, bbx):
        ...
        # RoIPool输入：(特征图, RoI框)，RoI框格式：(batch_id, x1, y1, x2, y2)
        out1 = self.roi1(feature1, bbx[:, :5].float())  # bbx转float避免类型错误
        out2 = self.roi2(feature2, bbx[:, :5].float())
        out = out1 + out2  # 融合两个RoI特征

        # TODO 13: 使用全连接层进行旋转角度的预测
        # 展平特征：(N, 512, 7, 7) → (N, 512*7*7)
        out_flat = out.view(out.shape[0], -1)
        out_lin1 = self.relu_lin(self.lin1(out_flat))  # 全连接+ReLU
        quaternion = self.lin2(out_lin1)  # 输出四元数：(N, 4*num_classes)
...
class PoseCNN(nn.Module):
    ...
    def __init__(self, pretrained_backbone, models_pcd, cam_intrinsic):
        ...
        self.num_classes = len(models_pcd)  # 类别数由点云模型数量决定

        # TODO 14: 初始化每个分支
        self.feature_extractor = FeatureExtraction(pretrained_backbone)  # 特征提取器
        self.segmentation_branch = SegmentationBranch(num_classes=self.num_classes)  # 分割分支
        ...
        self.TranslationBranch = TranslationBranch(num_classes=self.num_classes)  # 平移分支
    ...
    def forward(self, input_dict):
        if self.training:
            ...
            if filter_bbx_R.shape[0] != 0:
                # 旋转分支前向传播
                quater = self.RotationBranch(feat1, feat2, gt_bbx[:, :5])  
                gt_R = self.gtRotation(filter_bbx_R, input_dict)  
                pred_R, label = self.estimateRotation(quater, filter_bbx_R)  
                label = label.long()  
            ...
            with torch.no_grad():
                ...
                pred_centers, pred_depths = HoughVoting(
                    label=segmentation,  # 对应函数形参 label（分割结果）
                    centermap=trans_i,   # 对应函数形参 centermap（中心图/平移信息）
                    num_classes=self.num_classes  # 对应函数可选形参 num_classes
                )
